...
A landing page for your projects, inspired by =emacs-dashboard.el=. It shows recent changes, files you've been working on, and quick actions. It supports =.project.el= for metadata (e.g. name and description).

** But why?

I really like =emacs-dashboard=. I got rid of the quotes, and the header image, but boy is it functional! I find myself switching projects a lot. Initially I used =project.el='s switching menu. This displays in the mini-buffer and gives you options like find file, dired, and ripgrep. This was great, but more often than not I stared at the frame for a bit wondering where my project was, before I remembered to look in the mini-buffer.

What I wanted was something like =emacs-dashboard= but project-aware, which I I don't think =emacs-dashboard= is. Hopefully =project-dashboard= will fill that gap.
...
It's designed to feel like =emacs-dashboard= but scoped to your current project.
...
#+BEGIN_EXAMPLE
 ...
 Git: sqs_replay (g)
     unstaged M services/Cargo.lock
     unstaged M services/libs/fetcher/src/lib.rs
     unstaged M services/libs/graphql/src/state.rs

    0 staged, 7 unstaged, 18 untracked   ...more

 Recent Files: (r)
    1  services/libs/screenshot/src/screenshot_service.rs
    2  services/libs/db/src/jobs/job_service.rs
    3  services/libs/db/src/user/user_model.rs
    4  arwen-database-model.mwb

 Quick Actions: (a)
    f  Find file - C-c p f
    s  Search (ripgrep) - C-c s
    m  Magit status - C-x g
    d  Dired - C-x d
    e  Eshell - C-c p e
